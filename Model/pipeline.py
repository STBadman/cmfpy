...
def get_magneto(date:str, delim:str=':', return_name:bool=False):
    ...
    if not os.path.exists(f'{datadir}/{dataname}'):
        ...
        data_url = listhtml(url=url,contains=f'{datestr}')[0]
...
def adapt2pfsspy(filepath:str, #must already exist on your computer
    ...
    type_check(locals(),adapt2pfsspy)
...
def find_close_magneto_date(date:str,
    ...
    type_check(locals(),find_close_magneto_date)

    year, month, day, time = np.array(date.split(delim),dtype=str)

    url = f'https://gong.nso.edu/adapt/maps/gong/{year}/'
    files = listhtml(url, contains=year, include_url=False)
...
class CoronalModel:
    def __init__(self,
        ...
        type_check(locals(),CoronalModel.__init__)
        
        self.date = date
        self.delim = delim
        self.datetime = parse_to_datetime(self.date,delim=self.delim)
        ...
        self.datetime_model = parse_to_datetime(self.modeldate)
...
# %%
RSS = [1.5,2,2.5,3]
date = '2014:11:06:0000'
Model = CoronalModel(date,rss='hi')
...
for rss in RSS:
    Model = CoronalModel(date,rss=rss)
    p,r,f = Model.chmetric()
    wl = Model.wlmetric()
    nl = Model.nlmetric()
...
# %%
plt.figure()
plt.plot(RSS,F,label='CH Metric')
plt.plot(RSS,NL,label='NL Metric')
plt.plot(RSS,WL,label='WL Metric')
plt.plot([2.5,2.5],[0,1],c='k',ls='--')
plt.ylim(0,1)
plt.xlabel(r'$R_{\rm SS}/R_{\odot}$')
plt.ylabel('Score')
plt.title(r'Metrics vs $R_{\rm SS}$ - 2014:11:06')
plt.legend()

# %%
# strtodatetime

#euvmap with coronal holes and comparison
#lasco
#3d
#star plot
#contours